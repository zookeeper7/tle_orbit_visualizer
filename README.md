...
- **Viewer config** — 2D default, `resolutionScale: 1.0` on every modern phone (only `hardwareConcurrency <= 2` legacy devices fall back to 0.75 as a safety net), MSAA 4 on low-end / 8 on mid-range and up (Firefox forced to 1 for an upstream artifact; Cesium silently downgrades to the driver max), fog / atmosphere / sun / moon off, `tileCacheSize: 200`, `maximumScreenSpaceError: 1.5` (desktop-class globe detail), ground-station coverage rings drawn with a 2 px outline. `requestRenderMode` caps the average GPU duty to ~30 FPS during playback so the native-resolution + higher-MSAA budget still leaves interaction headroom.
