...
- **드래그**: 시트 위쪽의 회색 핸들을 손가락으로 잡고 위/아래로 끌면 시트가 손가락을 그대로 따라옵니다. 손을 떼면 가장 가까운 snap point 로 부드럽게 settling 합니다. 드래그 중에는 페이지 스크롤이 일어나지 않도록 핸들에 `touch-action: none` 이 적용되어 있습니다.
- **탭**: 같은 핸들을 짧게 탭하면 다음 snap point 로 순환합니다 (peek → half → full → peek). 5 px 미만의 이동은 탭으로 간주됩니다. 상단 바 오른쪽의 시트 토글 버튼도 동일한 cycle 동작을 수행합니다.
