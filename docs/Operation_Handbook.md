...
세 단계 snap point — **peek (96 px)**, **half (40 dvh)**, **full (상단 바 56 px 아래까지)** — 사이를 두 가지 방식으로 전환할 수 있습니다.

- **드래그**: 시트 위쪽의 회색 핸들을 손가락으로 잡고 위/아래로 끌면 시트가 손가락을 그대로 따라옵니다. **손을 떼면 그 위치 그대로 멈춥니다 — snap 없음**. peek 상태에서 살짝 위로 끌면 시트는 살짝만 올라옵니다. 드래그 중에는 페이지 스크롤이 일어나지 않도록 핸들에 `touch-action: none` 이 적용되어 있습니다.
- **탭**: 같은 핸들을 짧게 탭하면 다음 snap point 로 순환합니다 (peek → half → full → peek). 5 px 미만의 이동은 탭으로 간주됩니다. 드래그로 임의 위치에 시트를 두었더라도, 그 다음 탭은 **현재 위치에서 가장 가까운 snap 의 다음 snap** 으로 이동합니다. 상단 바 오른쪽의 시트 토글 버튼도 동일한 cycle 동작을 수행합니다.

full 상태에서도 상단 바를 절대 가리지 않도록 `max-height` 가 56 px 만큼 보정되어 있습니다.
