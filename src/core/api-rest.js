/**
 * REST backend — talks to the Express + SQLite server (server.js) on /api/*.
 *
 * Used when the app is run with `npm run server` + `npm run dev` (or a
 * production build that proxies /api to the Node server). All endpoints
 * here mirror the routes defined in server.js.
 *
 * The static-site demo build swaps this implementation out for the
 * localStorage adapter in api-local.js. See api.js for the switch.
 */
